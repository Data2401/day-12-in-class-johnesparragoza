...
All of the functions we've been using to make summaries can be used in summarize! `mean`, `median`, `var`, `sd`, `min`, `max`. And since we can use summarize on the groupings... now we can do this more powerfully. 
...
What if we want the same summary function on multiple variables? `across()` is a way to do this. Let's take the average (mean) values of a few of the size variables for size and species.
...
Let's take the average and median values of a few of the size variables for sex and species.
...
penguins %>%
  group_by(sex, species) %>%
  summarise(  across(ends_with("mm"), list(mean = mean, median = median), .names = "{.col}_{.fn}_by_sex_and_species")) 
                                                                                  #inserting column & function name
```
